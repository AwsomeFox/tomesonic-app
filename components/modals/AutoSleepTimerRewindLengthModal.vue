  ...
  <modals-modal v-model="show" :width="200" height="100%">
    <template #outer>
      <div class="absolute top-11 left-4 z-40">
        <p class="text-on-surface text-2xl truncate capitalize">{{ $strings.LabelAutoRewindTime }}</p>
      </div>
    </template>
      ...
      <div class="w-full overflow-x-hidden overflow-y-auto bg-surface rounded-lg border border-outline-variant shadow-elevation-4 backdrop-blur-md" style="max-height: 75%" @click.stop>
        ...
        <ul v-else class="h-full w-full" role="listbox" aria-labelledby="listbox-label">
          <template v-for="timeout in timeouts">
            <li :key="timeout" class="text-on-surface select-none relative py-4 cursor-pointer hover:bg-surface-container" role="option" @click="clickedOption(timeout)">
              <div class="flex items-center justify-center">
                <span class="font-normal block truncate text-lg">{{ timeout }} min</span>
              </div>
            </li>
          </template>
          <li class="text-on-surface select-none relative py-4 cursor-pointer hover:bg-surface-container" role="option" @click="manualTimerModal = true">
            <div class="flex items-center justify-center">
