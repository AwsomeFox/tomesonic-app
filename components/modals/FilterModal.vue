<template>
  <modals-modal v-model="show" width="90%" height="100%">
    <div class="w-full h-full overflow-hidden absolute top-0 left-0 flex items-center justify-center" data-modal-backdrop>
      <div class="w-full overflow-x-hidden overflow-y-auto bg-surface-container-high rounded-3xl border border-outline-variant border-opacity-40 shadow-elevation-4 mt-8" style="max-height: 75%">
        <!-- Material 3 Modal Header -->
        <div class="px-6 pt-5 pb-3 flex items-center">
          <span class="material-symbols text-on-surface mr-3" style="font-size: 24px">filter_list</span>
          <h2 class="text-headline-small text-on-surface font-medium flex-grow">{{ $strings.LabelFilter }}</h2>
          <ui-btn v-show="selected !== 'all'" size="sm" variant="text" class="h-9" :padding-y="0" @click="clearSelected">{{ $strings.ButtonClearFilter }}</ui-btn>
        </div>

        <ul v-show="!sublist" class="w-full px-2 pb-3 flex flex-col gap-1" role="listbox" aria-labelledby="listbox-label">
          <template v-for="item in items">
            <li :key="item.value" class="text-on-surface select-none relative py-3 px-3 pr-9 rounded-2xl cursor-pointer state-layer" :class="item.value === selected ? 'bg-primary-container text-on-primary-container' : ''" role="option" @click="clickedOption(item)">
              <div class="flex items-center justify-between">
                <span class="font-normal block truncate text-base">{{ item.text }}</span>
              </div>
              <div v-if="item.sublist" class="absolute right-2 top-0 bottom-0 h-full flex items-center">
                <span class="material-symbols text-on-surface-variant" style="font-size: 22px">chevron_right</span>
              </div>
            </li>
          </template>
        </ul>
        <ul v-show="sublist" class="w-full px-2 pb-3 flex flex-col gap-1" role="listbox" aria-labelledby="listbox-label">
          <li class="text-on-surface select-none relative py-3 px-3 rounded-2xl cursor-pointer state-layer" role="option" @click="sublist = null">
            <div class="flex items-center">
              <span class="material-symbols mr-2 text-on-surface-variant" style="font-size: 22px">arrow_back</span>
          ...
          <li v-if="!sublistItems.length" class="text-on-surface-variant select-none relative px-2" role="option">
            <div class="flex items-center justify-center">
              <span class="font-normal block truncate py-5 text-base">No {{ sublist }} items</span>
            </div>
          </li>
          <template v-for="item in sublistItems">
            <li :key="item.value" class="text-on-surface select-none relative py-3 px-3 rounded-2xl cursor-pointer state-layer" :class="`${sublist}.${item.value}` === selected ? 'bg-primary-container text-on-primary-container' : ''" role="option" @click="clickedSublistOption(item.value)">
              <div class="flex items-center">
