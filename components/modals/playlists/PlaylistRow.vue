<template>
  <div :key="playlist.id" :id="`playlist-row-${playlist.id}`" class="flex items-center px-3 py-2 justify-start relative border-y border-outline-variant border-opacity-20" :class="inPlaylist ? 'bg-primary-container text-on-primary-container' : 'hover:bg-surface-container transition-colors duration-200'">
