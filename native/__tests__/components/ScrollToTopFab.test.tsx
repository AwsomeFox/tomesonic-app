...
describe("ScrollToTopFab", () => {
  ...
  it("reduced motion: jumps in with no spring and unmounts immediately on exit", async () => {
    ...
    try {
      ...
      // Shown, but without running the entrance spring.
      expect(screen.getByLabelText("Scroll to top")).toBeTruthy();
      expect(spring).not.toHaveBeenCalled();

      // Hiding it unmounts right away — no collapse timing, no mounted-through-exit.
      await act(async () => {
        rerender(<ScrollToTopFab visible={false} onPress={jest.fn()} bottom={16} />);
      });
      expect(screen.queryByLabelText("Scroll to top")).toBeNull();
