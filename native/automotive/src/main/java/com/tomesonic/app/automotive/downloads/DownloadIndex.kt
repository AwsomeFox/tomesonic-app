 ...
 *
 * DataStore owns every other persisted value in this module (see
 * CredsRepository); this one is a plain file on purpose. It is read by a WORKER
 * process path and by the browse path, it can grow to hundreds of KB of track
 * metadata, and — the deciding reason — the phone has proven this exact file
 * discipline in `utils/autoCreds.ts`, where a native Android Auto service reads
 * a JSON array that JS rewrites underneath it.
 *
 ...
 *    car's cold start is already on a 10-second budget (DR-2).
 *  - **Never throw**: a corrupt file is quarantined (`.bad`) and the car behaves
 *    like a fresh install. Downloads are re-derivable; a crash loop on every
 *    Media Center bind is not recoverable by the user — there is no launcher
 *    icon to clear data from.
 *
...
class DownloadIndex(private val file: File) {
     ...
     * genuinely cannot suspend: playback resolves a downloaded book from a plain
    ...
    private suspend fun mutate(transform: (List<DownloadEntry>) -> List<DownloadEntry>) {
        mutex.withLock {
            ...
            // In-memory first: the browse tree is correct even if the disk write
            // fails (a full head unit), and the next successful write repairs
            // the file.
            state.value = next
    ...
    private fun writeToDisk(entries: List<DownloadEntry>) {
        ...
        try {
            ...
            // taking down a download worker or a browse collector for.
        }
    }
