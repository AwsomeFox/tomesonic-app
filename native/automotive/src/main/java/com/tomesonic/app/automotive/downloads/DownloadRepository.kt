...
/**
 * What a browse row can say about one entry — a book or one episode. Additive to
 * the frozen cross-wave interface; the tree renders it via the download badge
 * extra (ARCHITECTURE.md §4.3), nothing else depends on it.
 */
sealed class DownloadStatus {
    data object NotDownloaded : DownloadStatus()

    /** Enqueued and waiting on its constraint (unmetered Wi-Fi). */
...
/**
 * The downloads API every other part of the module talks to: the frozen surface
 * ported from the wear donor plus [status] and [cancel], which are additive.
 *
 ...
 * what keeps it callable from any coroutine.
 *
 ...
 * single-argument signatures delegate with a null episode, so they keep meaning
 * exactly "the book" and nothing else.
 *
...
class DownloadRepository(
    ...
    /**
     * Additive, non-suspending sibling of [entryFor]. Playback's
     ...
     * downloaded" and would silently stream a book that is on the head unit — so
     ...
     * is the item id, exactly as before. Deliberately NOT suspending — playback
     ...
     *
     * Default constraint is the one ARCHITECTURE.md §3 names for a car:
     * **unmetered network only**. The donor also required the charger; a car is
     * always powered while it can download at all, so that requirement would only
     * ever mean "never runs". Unmetered stays, and it is the load-bearing half —
     * a head unit's data plan is metered by default and a gigabyte of audiobook
     * over it is a bill, not a feature.
     *
     * [force] is "download it now": the network requirement relaxes to CONNECTED
     * rather than disappearing — a car on a tethered hotspot with no home Wi-Fi
     * is precisely who needs to force, and a download with no network constraint
     * at all would just fail instead of waiting.
     *
    ...
    /**
     * Additive: one item's state, folded from WorkManager's live job and the
     * index. Live work reports itself (a re-run over an existing entry really is
     * downloading); everything else defers to the index, because WorkManager
     * prunes finished jobs and their absence must never un-download a book that
     * is sitting on the head unit.
     */
    ...
    private fun workInfos(entryId: String): Flow<List<WorkInfo>> = flow {
        val upstream = try {
            ...
            // still answers downloaded-or-not, which is what the tree badges on.
            flowOf(emptyList())
        }
        emitAll(upstream)
    }
    ...
    /**
     * The ONE deliberate behaviour change from the donor (ARCHITECTURE.md §3):
     * no `setRequiresCharging`. The override mechanism is unchanged — [force]
     * still relaxes the network requirement to CONNECTED, and that is now the
     * only thing it relaxes.
     */
