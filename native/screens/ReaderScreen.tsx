    ...
    try {
      ...
      window.getReaderText = function(){
        try {
          ...
          post({ type: 'ttsText', text: String(t || '').replace(/\\s+/g, ' ').trim().slice(0, 8000) });
        } catch(e) { post({ type: 'ttsText', text: '' }); }
      };
...
export default function ReaderScreen({ route, navigation }: any) {
  ...
  // Stop any in-flight speech when leaving the reader.
  useEffect(() => {
    return () => {
      try {
      ...
      if (data.type === "location") {
        ...
        if (Speech && text) {
          ...
          try {
            Speech.speak(text, {
              onDone: () => {
                // Advance to the next page and continue if still playing.
  ...
  const toggleTts = () => {
    ...
    // Kick off: ask the WebView for the current section's text; the ttsText
    // message handler starts speaking and advances page-by-page.
    if (webRef.current) {
    ...
    <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 12, paddingTop: 8, paddingBottom: 8 }}>
      ...
      {isFoliateFormat && canRenderEbook ? (
        <View style={{ flexDirection: "row", alignItems: "center", columnGap: 4 }}>
          ...
          >
            <Icon name={ttsPlaying ? "pause" : "play"} size={24} color={ttsPlaying ? colors.primary : colors.onSurface} />
          </Pressable>
          <Pressable
          ...
          >
            <Icon name="bookmark" size={24} color={colors.onSurface} />
          </Pressable>
          <Pressable
      ...
      <BottomSheet visible={!!selection} onClose={() => setSelection(null)} showHandle={false}>
        <View style={{ padding: 20, paddingBottom: 20 }}>
          ...
          <View style={{ flexDirection: "row", columnGap: 12 }}>
            ...
            >
              <Icon name="search" size={18} color={colors.onSecondaryContainer} />
              <Text style={{ color: colors.onSecondaryContainer, fontSize: 14, fontWeight: "600" }}>Look up</Text>
            </Pressable>
            <Pressable
            ...
            >
              <Icon name="bookmark" size={18} color={colors.onSecondaryContainer} />
