...
import { resolveAudioTarget, audioPositionForReadingFraction, approximateClock, canJumpToFraction } from "../utils/formatSwitch";
...
// The live text-settings CSS pushed into an already-open book (font size /
...
${FOLIATE_BUNDLE}
  ...
  async function init() {
    try {
      ...
      // EXPLICIT user jump: applied as-is (forward OR backward).
      window.goToFraction = (f) => { try { view.goToFraction(f); } catch(e) {} };
      // Forward-only fraction seek for the AUTOMATIC linked catch-up (#P1/#P2):
      // when listening has run ahead of reading, pull the reader forward to the
      // audio fraction — but ONLY if it's meaningfully ahead of the reader's
      // REAL rendered position (compared here, where lastLocation.fraction is
      // known, AFTER the saved CFI restore). Never drags reading backward.
      window.seekForwardToFraction = (f) => {
        try {
          var target = Number(f);
          if (!isFinite(target)) return;
          var cur = lastFraction;
          try {
            if (view.lastLocation && typeof view.lastLocation.fraction === 'number') {
              cur = view.lastLocation.fraction;
            }
          } catch(e) {}
          if (target > cur + 0.001) view.goToFraction(target);
        } catch(e) {}
      };
      ...
      // ---- TTS: hand the text of the CURRENT PAGE to RN. ----
      ...
      // whose .range is a DOM Range spanning the CURRENTLY VISIBLE page; we read
      // from its start to its END so each utterance batch is exactly this page.
      // Reading to the end of the SECTION here (only clamping the start) made
      // the TTS advance — window.goNext() turns ONE page — re-request
      // cursor→end-of-section again, so a multi-page chapter was re-read ~N
      // times with shrinking overlap (#R1). Bounding the range END to the page
      // makes goNext advance cleanly with no re-read. If the page's end bound
      // can't be resolved we fall back to end-of-section, then whole-section
      // text, rather than breaking read-aloud.
          ...
          if (body) {
            ...
            if (range && range.startContainer && doc.createRange) {
              try {
                ...
                // START at the current cursor (top of the visible page). A range
                // from another section would throw; the catch below falls back.
                r.setStart(range.startContainer, range.startOffset || 0);
                // END at the END of the visible page — NOT the end of the
                // section — so goNext advances one page's worth at a time (#R1).
                // If setEnd fails, r keeps selectNodeContents' body-end (the old
                // whole-tail fallback), so read-aloud never breaks.
                if (range.endContainer) {
                  try { r.setEnd(range.endContainer, range.endOffset || 0); } catch(e2) {}
                }
                t = r.toString() || '';
              } catch(e1) { t = ''; }
            }
...
export default function ReaderScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    ...
    if (ebookStatus === "ready" && webRef.current) {
      const cleanCSS = buildLiveReaderCSS(fontSize, fontFamily, lineHeight, bg, fg);
  ...
  const onWebMessage = useCallback((event: any) => {
    try {
      ...
      if (data.type === "location") {
        ...
        // R2: flush the FULL current settings snapshot now that init() has
        // actually defined window.setReaderStyles/Theme/Margin/Flow/PageCurl.
        // The per-setting live-apply effects gate on ebookStatus === "ready",
        // which flips when the WebView MOUNTS — before those helpers exist — so
        // any setting changed during that mount→ready window no-op'd (guarded by
        // `window.setX &&`) and was never retried. Re-applying here on the
        // 'ready' MESSAGE (the same reliable signal the jump handoff uses) makes
        // those settings stick. The per-setting effects still handle later live
        // changes.
        if (webRef.current) {
          const cleanCSS = buildLiveReaderCSS(fontSize, fontFamily, lineHeight, bg, fg);
          webRef.current.injectJavaScript(
            `window.setReaderStyles && window.setReaderStyles(\`${cleanCSS}\`);` +
              `window.setReaderTheme && window.setReaderTheme(${JSON.stringify(bg)}, ${JSON.stringify(fg)});` +
              `window.setReaderMargin && window.setReaderMargin(${Number(readerMargin)});` +
              `window.setReaderFlow && window.setReaderFlow(${JSON.stringify(readerFlow)});` +
              `window.setPageCurl && window.setPageCurl(${pageCurl ? "true" : "false"});true;`
          );
        }
        // Player → reader handoff / linked catch-up, once the book is rendered.
        const f = pendingJumpRef.current;
        if (f != null && webRef.current) {
          // EXPLICIT "Read from here" (PlayerBottomSheet): apply the mapped
          // fraction as-is — a deliberate user jump, forward OR backward. This
          // overrides the startCfi restore.
          pendingJumpRef.current = null;
          webRef.current.injectJavaScript(
            `window.goToFraction && window.goToFraction(${Number(f)});true;`
          );
        } else if (webRef.current) {
          // AUTOMATIC linked catch-up (#P1/#P2): when this book's progresses are
          // LINKED and the format supports fraction jumps, seek the reader
          // FORWARD to the audio listening fraction if it's meaningfully ahead
          // of the reader's REAL restored position. Forward-only — the WebView
          // compares against its actual rendered fraction (never backward). The
          // reader self-handling this fixes EVERY entry point (ItemDetail,
          // Bookshelf, Library, Series, Playlist) at once, and keys off the true
          // rendered page rather than a reconciled percentage that ItemDetail
          // may have already bumped to match (which defeated the old gate).
          try {
            const linked = !!useUserStore.getState().settings?.linkedProgress?.[itemId];
            if (linked && canJumpToFraction(ebookFormat)) {
              const prog = useUserStore.getState().mediaProgress?.[itemId];
              const audioFraction = Math.max(0, Math.min(1, Number(prog?.progress) || 0));
              if (audioFraction > 0) {
                webRef.current.injectJavaScript(
                  `window.seekForwardToFraction && window.seekForwardToFraction(${audioFraction});true;`
                );
              }
            }
          } catch {}
        ...
        const speakFrom = (i: number) => {
          ...
          try {
            Speech.speak(chunks[i], {
              ...
              // expo-speech's native callbacks fire ASYNCHRONOUSLY — the unmount
              // cleanup calls Speech.stop(), which resolves onStopped AFTER the
              // screen is gone. Guard every callback with unmountedRef so they
              // no-op instead of setState-ing on an unmounted component (#R6).
              onDone: () => {
                if (unmountedRef.current) return;
    ...
    // fontSize/…/pageCurl + ebookFormat are read by the on-`ready` settings
    // flush (#R2) and the linked catch-up (#P1/#P2); include them so `ready`
    // fires against the CURRENT snapshot rather than a stale closure.
  }, [
    progressKey,
    itemId,
    highlightsKey,
  ...
  const Header = (
    <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 12, paddingTop: 8, paddingBottom: 8, backgroundColor: chromeBg }}>
      ...
      <Text accessibilityRole="header" numberOfLines={1} style={{ color: chromeFg, fontSize: 18, fontWeight: "700", marginLeft: 4, flex: 1 }}>
        {title || "Reader"}
      </Text>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: chromeBg }} edges={["top", "left", "right"]}>
      ...
      <BottomSheet visible={showToc} onClose={() => setShowToc(false)} showHandle={false}>
        <View style={{ paddingBottom: 16 }}>
            ...
            >
              <Text accessibilityRole="header" style={{ color: colors.onSurface, fontSize: 18, fontWeight: "700" }}>Table of Contents</Text>
              <Pressable
      ...
      <BottomSheet visible={showSearch} onClose={() => setShowSearch(false)} showHandle={false}>
        <View style={{ padding: 16, paddingBottom: 16 }}>
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <Text accessibilityRole="header" style={{ color: colors.onSurface, fontSize: 18, fontWeight: "700" }}>Search in Book</Text>
            <Pressable
