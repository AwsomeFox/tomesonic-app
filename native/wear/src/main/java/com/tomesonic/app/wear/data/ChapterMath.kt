 ...
 * can be pinned by plain JVM tests, exactly like the TS original: the scrubber,
 * the chapter title, prev/next chapter and the progress syncer all derive from
 * these two mappings, and a boundary fixed in one copy but not the others is
 * precisely the desync the TS extraction existed to prevent.
 *
...
object ChapterMath {
     ...
     * absolute there. The watch always builds one media3 item PER TRACK, so a
