...
class DownloadWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            ...
            // perfectly good download, and the offline UI falls back to a
            // placeholder exactly like the online one does.
            ...
            // Cancelled by the user, or stopped because a constraint lapsed
            // (unplugged, off WiFi). Drop the half-written .part files but KEEP
            // whole tracks — re-enqueueing then resumes from here instead of
     ...
     * the MERGED manifest. In every one of those cases the bytes still want to
    ...
    private suspend fun fetch(plan: PlannedDownload, onBytes: suspend (Long, Long) -> Unit) {
        ...
        val url = client.resolve(plan.url) ?: throw IOException("watch is not configured")
    ...
    /**
     * A small retry cap, then give up. The retry is what covers a watch that
     * dropped off WiFi mid-book; past that the failure is real (a deleted item,
     * an expired token) and hammering it costs battery for nothing.
     */
    ...
    companion object {
        ...
        fun buildPlan(item: ItemDetail, dir: File, coverUrl: String?): DownloadPlan? {
            ...
            // everything else, so AbsApi's url (no token query param) is right.
            val cover = coverUrl
        ...
        fun foregroundInfo(
            ...
            val notification = Notification.Builder(context, CHANNEL_ID)
                ...
                // A platform drawable: the wear module ships no notification
                // icon of its own, and a missing small icon throws at post time.
                .setSmallIcon(android.R.drawable.stat_sys_download)
                .setOngoing(true)
                .setOnlyAlertOnce(true)
                // Watch-local: a download running on the wrist has no business
                // bridging a notification to the phone.
                .setLocalOnly(true)
