...
/**
 * The watch's offline progress store — a Kotlin port of the phone's proven
 ...
 *     `wear-local_<itemId>[-<episodeId>]_<YYYY-MM-DD>`, flushed one
...
class OfflineProgressQueue(
     ...
     * callers read it as "the watch is caught up", never as a retry signal (the
    ...
    private suspend fun deviceInfo(): JSONObject =
        ...
        // the SAME device, or ABS files this watch's stats under two of them.
    ...
    private fun dayOfWeekOf(ts: Long): String {
        ...
        // array by JS getDay() (0=Sunday). `% 7` bridges the two so both devices
        // label the same day identically in ABS's stats.
