 ...
 * bill a paused watch for hours in a pocket.
 *
 ...
 * and getting it wrong is invisible on the watch and permanent on the server.
 */
internal class ListenAccumulator(private val maxIntervalSeconds: Double = MAX_INTERVAL_SECONDS) {
    ...
    fun update(nowMs: Long, isPlaying: Boolean) {
        ...
        if (since != null) {
            ...
            // Capped: a deferred coroutine (doze, a suspended process) can stretch
            // one interval arbitrarily, and an hour banked from a single tick is
            // never a real reading.
 ...
 * counted: the seconds are claimed once, and ABS keys the queued record under a
 * `wear-local_` id that upserts separately from the streaming session.
 *
...
class ProgressSyncer(
     ...
     * client's read timeout whenever the watch is offline.
     */
    suspend fun handOff() {
        val claimed = claimFinal() ?: return
     ...
     * service stopped while paused mid-book.
     */
    fun finishDetached() {
        stopTicking()
    ...
    private fun scheduleSeekSync() {
        // Debounced: rotary scrubbing fires a discontinuity per detent, and each
    ...
    private suspend fun deliver(claimed: Claim, closing: Boolean) {
        ...
        if (sessionId != null) {
            ...
            if (delivered) {
                ...
                // the queued `wear-local_` record upserts independently of the
                // streaming session, so banking them here can't double count.
    ...
    companion object {
        ...
        /** One request per scrub gesture, not one per rotary detent. */
